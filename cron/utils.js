...
const fieldOrder = ['second', 'minute', 'hour', 'dayOfMonth', 'month', 'dayOfWeek'];
...
module.exports.convertToCronExpression = (schedule) => {
    ...
    for (const fieldName of fieldOrder) {
        if (transformedSchedule[fieldName] === undefined) {
            if (!oneFieldAlreadySet) {
                transformedSchedule[fieldName] = DEFAULT_MISSING_VALUES_FOR_FIELDS[fieldName];
            } else {
    ...
    let { second, minute, hour, dayOfMonth, month, dayOfWeek } = transformedSchedule;
